(ns infura.main
  "Kotodama WASM entrypoint for the Infura clean-room actor (L4) — Clojure port.
  ...
  facts (`infura.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "infura")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"infura.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "infura-compat" :tier tier :entities entities} 200])
...
(defn start! [] :infura-compat/ready)
